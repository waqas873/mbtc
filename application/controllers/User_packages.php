...
class User_packages extends CI_Controller
{
	...
	*/
	public function index($status = '')
	{
		...
		if(!ctype_digit($status) || $status > 3){
			redirect("user/UserDashboard"); exit;
		}
        
        $where = ($status==3)?"up_id > 0":"up_status = '".$status."'";
        $joins = array(
        ...
        ($status == 3)?$this->selected_tab = "all_packages":'';
        ($status == 0)?$this->selected_tab = "pending_packages":'';
        ($status == 1)?$this->selected_tab = "purchased_packages":'';
        ($status == 2)?$this->selected_tab = "rejected_packages":'';

        ($status == 3)?$data['heading'] = "All Packages":'';
        ($status == 0)?$data['heading'] = "Pending Packages":'';
        ($status == 1)?$data['heading'] = "Purchased Packages":'';
        ($status == 2)?$data['heading'] = "Rejected Packages":'';
        
	  	$this->load->view("user_packages/index",$data);
	}
