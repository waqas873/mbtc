...
class User extends CI_Controller {
    ...
    public function tree_view(){
        ...
        $this->load->view("users/tree_view",$data);
    }
    ...
    public function get_tree_view(){
        ...
        foreach ($users_ids as $index=>$user_id){
            ...
            $packageDe = package_detail($user_id);
    ...
    {   
        ...
        if($this->input->post()){
            ...
            if(!empty($result)){
                ...
                $sponsor_id = 'Null';
                $where = "id = '".$result['referral_id']."' ";
                $sponsor = $this->users->get_where('*', $where, true, '' , '', '');
                if(!empty($sponsor)){
                    $sponsor_id = $sponsor[0]['sponsor_code'];
                }

                $where = "referral_id = '".$user_id."' AND position = 'left' ";
                $direct_left = $this->users->count_rows($where);
                $where = "referral_id = '".$user_id."' AND position = 'right' ";
                $direct_right = $this->users->count_rows($where);
                ...
                $where = "point_user_id = '".$user_id."' AND point_date = '".date('Y-m-d')."'";
                $curr_points = $this->points->get_where('SUM(left_points) as left_points,SUM(right_points) as right_points', $where, true, '' , '', '');
                $curr_points = $curr_points[0];
                $curr_left_points = ($curr_points['left_points']>0)?$curr_points['left_points']:0;
                $curr_right_points = ($curr_points['right_points']>0)?$curr_points['right_points']:0;

                $left_users = $this->left_right_users($user_id,"left");
                $right_users = $this->left_right_users($user_id,"right");
                $total_users = $left_users+$right_users;
                $detail = '<div class="row all_rows"><div class="col-sm-6"><span class="strong_text">User ID</span></div><div class="col-sm-6">'.$result['sponsor_code'].'</div></div><div class="bottom_border"></div>';
                $detail .= '<div class="row all_rows"><div class="col-sm-6"><span class="strong_text">Full Name</span></div><div class="col-sm-6">'.ucwords($fullname).'</div></div><div class="bottom_border"></div>';
                $detail .= '<div class="row all_rows"><div class="col-sm-6"><span class="strong_text">Sponsor ID</span></div><div class="col-sm-6">'.$sponsor_id.'</div></div><div class="bottom_border"></div>';
                $detail .= '<div class="row all_rows"><div class="col-sm-4"><span class="strong_text">Position</span></div><div class="col-sm-4 strong_text">Left</div><div class="col-sm-4 strong_text">Right</div></div><div class="bottom_border"></div>';
                $detail .= '<div class="row all_rows"><div class="col-sm-4"><span class="strong_text">Total Bussiness</span></div><div class="col-sm-4">$'.$left_points.'</div><div class="col-sm-4">$'.$right_points.'</div></div><div class="bottom_border"></div>';
                $detail .= '<div class="row all_rows"><div class="col-sm-4"><span class="strong_text">Current Bussiness</span></div><div class="col-sm-4">$'.$curr_left_points.'</div><div class="col-sm-4">$'.$curr_right_points.'</div></div><div class="bottom_border"></div>';
                $detail .= '<div class="row all_rows"><div class="col-sm-4"><span class="strong_text">Total Members</span></div><div class="col-sm-4">'.$left_users.'</div><div class="col-sm-4">'.$right_users.'</div></div><div class="bottom_border"></div>';
                $detail .= '<div class="row all_rows"><div class="col-sm-4"><span class="strong_text">Direct Members</span></div><div class="col-sm-4">'.$direct_left.'</div><div class="col-sm-4">'.$direct_right.'</div></div><div class="bottom_border"></div>';
                $data['result'] = $detail;
                $data['response'] = true;
            }
        }
